...
{
    std::vector<int> arraysSum;
    int endArrayLenght = arrayOfArrays[0].size() + shift * (arrayOfArrays.size() - 1);
    for(int k = 0; k < endArrayLenght; k++)
    {
      arraysSum.push_back(0);
    }
    ...
    for(int i = 0; i < arrayOfArrays.size(); i++)
    {
      ...
      {
        arraysSum[ j + shift * i] += arrayOfArrays[i][j];
      }
    }
